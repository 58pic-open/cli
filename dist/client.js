import { DEFAULT_BASE, saveConfig } from "./config.js";
...
export async function resolveCredentials(flags, file) {
    ...
    if (file.oauth?.accessToken) {
        ...
        if (file.oauth.refreshToken) {
            try {
                ...
                const refreshed = await refreshOAuthToken(baseUrl, file.oauth.refreshToken, file.oauth.clientId);
                const updated = { ...file.oauth, ...refreshed };
                await saveConfig({ oauth: updated });
                return { apiKey: updated.accessToken, baseUrl };
            }
            catch (e) {
                process.stderr.write(`Token 刷新失败（${e.message}），请重新执行 58pic auth login\n`);
                // 刷新失败，继续尝试 apiKey 兜底
            }
        }
        else {
            process.stderr.write("OAuth token 已过期，请重新执行 58pic auth login\n");
        }
    }
    ...
    throw new Error("未配置认证信息，请先执行 58pic auth login（OAuth）或 58pic config init（API Key）");
}
