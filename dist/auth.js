 ...
 * 失败时按约定模式构造 fallback。
 */
async function resolveEndpoints(apiBase) {
    const oauthBase = getOAuthBase(apiBase);
    try {
        const res = await fetch(`${oauthBase}/.well-known/oauth-authorization-server`, { signal: AbortSignal.timeout(5000) });
        if (res.ok) {
...
function createCallbackServer() {
    return new Promise((resolve, reject) => {
        ...
        server.listen(0, "127.0.0.1", () => {
            ...
            const waitForCode = (expectedState, timeoutMs = 120000) => new Promise((res, rej) => {
                    ...
                    try {
                        ...
                        if (error || !code || returnedState !== expectedState) {
                            const msg = error
                                ...
                                : "无效的回调参数";
                            resp.end(`<html><body style="font-family:sans-serif;padding:2em">` +
...
// ---- 动态客户端注册 ----
async function registerDynamicClient(registerUrl, redirectUri) {
...
// ---- 授权码换 token ----
async function exchangeCode(tokenUrl, code, clientId, redirectUri, verifier) {
...
export async function refreshOAuthToken(apiBase, refreshToken, clientId) {
...
// ---- 撤销 token ----
export async function revokeToken(apiBase, token) {
    const endpoints = await resolveEndpoints(apiBase);
...
// ---- 完整登录流程 ----
export async function loginWithOAuth(apiBase = DEFAULT_BASE) {
    const endpoints = await resolveEndpoints(apiBase);
    // 1. 启动本地回调服务器，拿到随机端口
    ...
    authUrl.searchParams.set("scope", "58pic:search 58pic:generate 58pic:download 58pic:credits:read");
    ...
    const tokens = await exchangeCode(endpoints.token, code, clientId, redirectUri, pkce.verifier);
    // 8. 持久化
