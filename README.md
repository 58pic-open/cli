...
- **Node.js** ≥ 18（内置 `fetch`）
- **Git** — 使用 `npm install -g github:58pic-open/cli`、`npx skills add https://github.com/58pic-open/cli` 等方式从 GitHub 拉取时，本机需已安装 **Git**，且 `git` 在终端 `PATH` 中可用（`git --version` 可执行）。仅从 npm 注册表安装 `@58pic/cli` 且不通过 GitHub 安装 Skills 时，可不依赖 Git。
...
| 项目 | 说明 |
|------|------|
| **安装命令** | `npx skills add https://github.com/58pic-open/cli -y -g`（见 [安装](#安装) 中的 **CLI 与 Agent Skills 配套安装**）。 |
...
```bash
# 仅列出本仓库提供的 Skill，不安装
npx skills add https://github.com/58pic-open/cli --list

# 只安装名为 58pic 的这一条（与 SKILL.md  frontmatter 中 name 一致）
npx skills add https://github.com/58pic-open/cli --skill 58pic -y -g

# 指定 Agent（可多选），例如只装到 Cursor
npx skills add https://github.com/58pic-open/cli --skill 58pic -y -g -a cursor
```
...
**Requirements:** Node.js ≥ 18. **Git** is required when installing from GitHub (`npm install -g github:58pic-open/cli`, `npx skills add https://github.com/...`); ensure `git` is on your `PATH`. Installing only `@58pic/cli` from the npm registry without GitHub-based flows may omit Git.

**Install (CLI):** From registry: `npm install -g @58pic/cli`. From GitHub: `npm install -g github:58pic-open/cli` (prebuilt `dist/` in repo; no compile at install). Local dev: `git clone`, `npm install`, `npm run build` after TS changes, use `58pic` (also `58pic-cli`).

**Install (Agent Skills):** use [vercel-labs/skills](https://github.com/vercel-labs/skills): `npx skills add https://github.com/58pic-open/cli -y -g`. List only: `npx skills add https://github.com/58pic-open/cli --list`. Single skill: `npx skills add https://github.com/58pic-open/cli --skill 58pic -y -g`. Skills live under [`skills/`](skills/) in this repo and are included in the npm package `files` field.
...
**Agents:** Install Skills with `npx skills add https://github.com/58pic-open/cli -y -g`, then run **`58pic` in the terminal** as documented in [`skills/58pic/SKILL.md`](skills/58pic/SKILL.md).
