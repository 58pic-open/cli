...
面向 [千图 AI 开放平台](https://ai.58pic.com/) 的命令行工具：本地配置、常用接口快捷命令、通用 `api` 透传。支持 `config init`、`auth status`、`--format`、`dry-run` 等与常见开放平台 CLI 相近的用法。**Agent Skills** 通过开源 [**skills** CLI](https://github.com/vercel-labs/skills)（`npx skills add …`）从本仓库 `skills/` 安装到 Cursor、Claude Code 等，见下文 **CLI 与 Agent Skills 配套安装**。
...
npm 会临时拉取仓库并在本地执行 `prepare`（即 `npm run build`），生成 `dist/` 后再链接全局命令：
...
# 只安装名为 qiantu-58pic 的这一条（与 SKILL.md  frontmatter 中 name 一致）
npx skills add 58pic-open/cli --skill qiantu-58pic -y -g

# 指定 Agent（可多选），例如只装到 Cursor
npx skills add 58pic-open/cli --skill qiantu-58pic -y -g -a cursor
```
...
> **给 AI 助手：** 先完成「CLI + Skills」两步安装，再配置 Key。详见上文 **CLI 与 Agent Skills 配套安装**。
...
| 项目 | 说明 |
|------|------|
| **安装命令** | `npx skills add 58pic-open/cli -y -g`（见上文 **CLI 与 Agent Skills 配套安装**）。 |
| **Skill 源文件** | [`skills/58pic/SKILL.md`](skills/58pic/SKILL.md)，frontmatter 中 `name` 为 **`qiantu-58pic`**（供 `npx skills add … --skill qiantu-58pic` 使用）。 |
...
**Install (CLI):** Node ≥ 18. From registry: `npm install -g @58pic/cli`. From GitHub without cloning: `npm install -g github:58pic-open/cli` (requires `prepare` / build on install). Local dev: `git clone`, `npm install`, use `58pic` (also `58pic-cli`).

**Install (Agent Skills):** use [vercel-labs/skills](https://github.com/vercel-labs/skills): `npx skills add 58pic-open/cli -y -g`. List only: `npx skills add 58pic-open/cli --list`. Single skill: `npx skills add 58pic-open/cli --skill qiantu-58pic -y -g`. Skills live under [`skills/`](skills/) in this repo and are included in the npm package `files` field.
