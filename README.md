...
- **Node.js** ≥ 18（内置 `fetch`）
- **Git** — 执行 `npx skills add 58pic/cli …`（从 GitHub 拉取 Skill）或 `npm install -g github:58pic-open/cli` 时，本机需已安装 **Git**，且 `git` 在 `PATH` 中可用。若只执行 `npm install -g @58pic/cli`、不安装 Skills，可不依赖 Git。
...
| 项目 | 说明 |
|------|------|
| **安装命令** | `npx skills add 58pic/cli -y -g`（见 [安装](#安装) 中的 **CLI 与 Agent Skills 配套安装**）。 |
...
- `npx skills` 由 [vercel-labs/skills](https://github.com/vercel-labs/skills) 提供；`58pic/cli` 为 GitHub `owner/repo` 简写，**需要本机已装 Git**。若克隆失败，可改用 `58pic-open/cli` 或完整地址 `https://github.com/58pic-open/cli`。
...
推荐顺序：**先 `npm install -g @58pic/cli`，再 `npx skills add 58pic/cli`**（与上文「推荐（npm 注册表 + Skills）」一致）。备选：CLI 也可从 `github:58pic-open/cli` 安装；Skills 亦可使用 `npx skills add 58pic-open/cli …` 或 `https://github.com/58pic-open/cli`（与短名指向同一仓库时效果相同）。
...
```bash
npx skills add 58pic/cli --list

npx skills add 58pic/cli --skill 58pic -y -g

npx skills add 58pic/cli --skill 58pic -y -g -a cursor
```
...
**Requirements:** Node.js ≥ 18. **Git** is required for `npx skills add 58pic/cli …` or `npm install -g github:58pic-open/cli`; ensure `git` is on your `PATH`. Global `npm install -g @58pic/cli` alone (no Skills from GitHub) may omit Git.
...
**Install (Agent Skills):** [vercel-labs/skills](https://github.com/vercel-labs/skills). If `58pic/cli` fails to clone, try `58pic-open/cli` or `https://github.com/58pic-open/cli`. List: `npx skills add 58pic/cli --list`. Single skill: `npx skills add 58pic/cli --skill 58pic -y -g`. Skills live under [`skills/`](skills/) and are included in the npm `files` field.
