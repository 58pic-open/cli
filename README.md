...
**获取 API Key：** 浏览器登录 [千图 AI](https://ai.58pic.com/) 后打开  
...
若无 Key，引导用户至 [获取 API Key 页面](https://ai.58pic.com/history?openHistory=1&historyType=5)（须先登录）。
...
在浏览器登录千图 AI 后打开：**[https://ai.58pic.com/history?openHistory=1&historyType=5](https://ai.58pic.com/history?openHistory=1&historyType=5)**  
取得 Key 后执行 `58pic config init`（或 `58pic config init --api-key "<key>"`）。具体入口以官网为准。
...
**API Key:** after signing in to Qiantu AI, open [this page](https://ai.58pic.com/history?openHistory=1&historyType=5) to obtain a key, then run `58pic config init`.
