...
import { loadConfig, saveConfig, maskKey, configPath, DEFAULT_BASE, } from "./config.js";
...
async function main() {
    ...
    program
        ...
        .action(async () => {
        const c = await loadConfig();
        const has = Boolean(c.apiKey ?? process.env["58PIC_API_KEY"]);
        console.log(JSON.stringify({
            loggedIn: has,
            apiKey: maskKey(c.apiKey ?? process.env["58PIC_API_KEY"]),
