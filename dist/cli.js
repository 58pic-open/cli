...
import { loadConfig, saveConfig, clearOAuth, maskKey, configPath, DEFAULT_BASE, DEFAULT_WORKFLOW_BASE, } from "./config.js";
...
function addGlobalOpts(cmd) {
    return cmd
        ...
        .option("-f, --format <fmt>", "输出：json | pretty | table", "pretty");
}
function fmtOf(s) {
    if (s === "json" || s === "pretty" || s === "table")
        return s;
    return "pretty";
}
async function getCtx(opts, file) {
    return resolveCredentials(opts, file);
}
function addWorkflowOpts(cmd) {
    return addGlobalOpts(cmd).option("--workflow-base-url <url>", `工作流 API 根地址，默认 ${DEFAULT_WORKFLOW_BASE}`);
}
function workflowBaseOf(opts, file) {
    return (opts.workflowBaseUrl ??
...
async function main() {
    ...
    authCmd
        ...
        .action(async (opts) => {
        const c = await loadConfig();
        const baseUrl = opts.baseUrl ?? process.env["58PIC_BASE_URL"] ?? c.baseUrl ?? DEFAULT_BASE;
        // 已有有效 token 时给出提示
        if (!opts.force && c.oauth?.accessToken) {
            const now = Math.floor(Date.now() / 1000);
            const expiresAt = c.oauth.expiresAt ?? Infinity;
            if (expiresAt > now) {
                console.error("已登录（OAuth）。如需重新授权请加 --force 参数。");
                return;
            }
        }
        try {
            ...
            console.error("OAuth 登录失败：", e.message);
            process.exitCode = 1;
        }
    });
    ...
    workflowGet.option("--version <id>", "指定历史画布版本").action(async (id, opts) => {
        const file = await loadConfig();
        const ctx = await getCtx(opts, file);
    ...
    workflowCreate
        ...
        const file = await loadConfig();
        const ctx = await getCtx(opts, file);
        const body = { name, description: opts.description };
        if (opts.canvasFile) {
    ...
    workflowSave.requiredOption("--canvas-file <path>", "完整画布 JSON 文件").action(async (id, opts) => {
        const file = await loadConfig();
        const ctx = await getCtx(opts, file);
        const workflowID = parseWorkflowID(id);
    ...
    authCmd
        ...
        const oauthValid = hasOAuth && (c.oauth.expiresAt == null || c.oauth.expiresAt > now);
    ...
    const modelCap = program
        ...
        .description("快捷：查询指定模型支持的比例（aspect）/ 清晰度（resolution）/ 时长（duration）选项")
        .argument("<model_id>", "模型 ID（可先用 58pic models 查询）");
    addGlobalOpts(modelCap);
    modelCap.action(async (modelId, opts) => {
        const file = await loadConfig();
        const ctx = await getCtx(opts, file);
    ...
    download.action(async (pid, opts) => {
        const file = await loadConfig();
        const ctx = await getCtx(opts, file);
    ...
    const genVideo = program
        ...
        .description("快捷：提交视频生成任务（文生视频 / 图生视频）；返回 ai_id 后可用 same-style-status 查进度");
    addGlobalOpts(genVideo);
    genVideo
        .requiredOption("-m, --model <id>", "模型 ID（可先 58pic models）")
        .option("--reference-url <url>", "参考图 URL（图生视频时传）")
        .option("--prompt <text>", "描述词")
        .option("--aspect <value>", "视频比例（如 \"16:9\"、\"9:16\"）或 model-capabilities 返回的 aspect ID")
        .option("--resolution <value>", "清晰度（如 \"1080p\"、\"720p\"、\"4K\"）或 resolution_options ID")
        .option("--duration <value>", "时长（如 \"5s\"、\"5秒\"、\"10\"）或 duration_options ID")
        .option("--end-frame-url <url>", "结束帧图 URL（部分模型支持）")
        .option("--body-file <path>", "JSON 文件，若指定则与其它体字段合并（文件优先覆盖同名键）")
        .argument("[prompt...]", "描述词（多词以空格拼接；与 --prompt 并存时 --prompt 优先）")
        .action(async (promptParts, opts) => {
        const file = await loadConfig();
        const ctx = await getCtx(opts, file);
        let extra = {};
        ...
        if (opts.aspect)
            body.Aspect = opts.aspect;
        if (opts.resolution)
            body.video_resolution = opts.resolution;
        if (opts.duration)
            body.video_duration = opts.duration;
        if (opts.referenceUrl)
            body.reference_image_url = opts.referenceUrl;
        if (opts.endFrameUrl)
            body.end_frame_url = opts.endFrameUrl;
        const positional = Array.isArray(promptParts)
            ? promptParts.join(" ").trim()
            : "";
        const flagPrompt = (opts.prompt ?? "").trim();
    ...
    apiCmd.action(async (route, opts) => {
        const file = await loadConfig();
        const ctx = await getCtx(opts, file);
    ...
    program
        ...
        console.log(JSON.stringify({ method: o.method.toUpperCase(), url }, null, 2));
    });
